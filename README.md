...
## Spør boten direkte i Telegram

I tillegg til den daglige automatiske sjekken kan du **skrive til boten** for å
sjekke en spesifikk destinasjon/år/sesong når du vil, f.eks.:

```
sjekk billetter Tokyo 2026
sjekk Dubai høst 2026
søk Chicago 2027
```

En egen GitHub Actions-jobb (`telegram-bot.yml`) sjekker for nye meldinger til
boten hvert 5. minutt, tolker kommandoen, søker mot awardhacks.se, og svarer
deg tilbake i samme Telegram-chat innen kort tid.

**Kjente byer:** Tokyo, Chicago, Washington, Atlanta, Boston, Los Angeles,
San Francisco, Seattle, Miami, New York, Toronto, Seoul, Mumbai, Bangkok,
Dubai, Phuket, Krabi, Stockholm, Oslo, København, Göteborg, Aalborg.

**Sesonger:** vår, sommer, høst, vinter (kombiner gjerne med årstall, f.eks.
"høst 2026" = september-november 2026).

Standard avreise er Skandinavia (CPH/ARN/OSL) med mindre du nevner en annen
skandinavisk by. Meldingen må inneholde ordet **"sjekk"** eller **"søk"** for
at boten skal reagere - vanlig prat i chatten ignoreres.

Denne funksjonen krever samme `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID`-secrets
som allerede er satt opp for den daglige sjekken (steg 2 over) - ingen ekstra
oppsett nødvendig utover å legge til `telegram-bot.yml`-workflowen og
`telegram_bot.py`-scriptet i repoet.
